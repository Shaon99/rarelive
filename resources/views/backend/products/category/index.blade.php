                    ...
                    <div class="card">
                        <div class="card-header">
                            @if (auth()->guard('admin')->user()->can('category_add'))
                                <button data-href="{{ route('admin.category.store') }}" data-name="Category"
                                    class="btn btn-primary create"><i class="fas fa-plus-circle"></i>
                                    {{ __('Create Category') }}</button>
                            @endif
                        </div>
                        <div class="card-body">
                                                ...
                                                <td>
                                                    @if (auth()->guard('admin')->user()->can('category_edit'))
                                                        <button class="btn btn-primary btn-sm edit mr-1"
                                                            data-href="{{ route('admin.category.update', $item->id) }}"
                                                            data-item="{{ $item->name }}" data-name="Category"
                                                    ...
                                                    @if (auth()->guard('admin')->user()->can('category_delete'))
                                                        <button class="btn btn-danger delete btn-sm"
                                                            data-href="{{ route('admin.category.destroy', $item->id) }}"
